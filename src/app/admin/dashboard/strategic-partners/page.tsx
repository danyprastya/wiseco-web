...
import { StrategicPartner, R2_FOLDERS } from "@/lib/db-types";
...
export default function StrategicPartnersPage() {
  ...
  const [selectedItem, setSelectedItem] = useState<StrategicPartner | null>(
    null
  );
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        "/api/data/strategic-partners?activeOnly=false"
      );
  ...
  const openDialog = (item?: StrategicPartner) => {
    if (item) {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const url = "/api/data/strategic-partners";
      ...
      toast.success(selectedItem ? "Partner updated!" : "Partner created!");
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `/api/data/strategic-partners?id=${
          selectedItem.id
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Strategic Partners</h1>
        <p className="text-gray-500 mt-1">
          Manage strategic partner logos displayed on the website.
        </p>
      </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              {selectedItem ? "Edit Partner" : "Add New Partner"}
            </DialogTitle>
            <DialogDescription>
              {selectedItem
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <ImageUpload
                ...
                folder={R2_FOLDERS.STRATEGIC_PARTNERS}
              />
            </div>
