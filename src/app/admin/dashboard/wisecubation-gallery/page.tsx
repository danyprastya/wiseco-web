...
import { WisecubationGalleryItem, R2_FOLDERS } from "@/lib/db-types";

export default function WisecubationGalleryPage() {
  const [items, setItems] = useState<WisecubationGalleryItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] =
    useState<WisecubationGalleryItem | null>(null);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        "/api/data/wisecubation-gallery?activeOnly=false"
      );
  ...
  const openDialog = (item?: WisecubationGalleryItem) => {
    if (item) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `/api/data/wisecubation-gallery?id=${
          selectedItem.id
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-1">
          Manage gallery images for the Wisecubation page.
        </p>
      </div>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <ImageUpload
                ...
                folder={R2_FOLDERS.WISECUBATION_GALLERY}
              />
            </div>
            <div className="space-y-2">
