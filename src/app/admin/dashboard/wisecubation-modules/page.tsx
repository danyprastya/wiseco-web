...
import { WisecubationModuleItem, R2_FOLDERS } from "@/lib/db-types";

export default function WisecubationModulesPage() {
  const [items, setItems] = useState<WisecubationModuleItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] =
    useState<WisecubationModuleItem | null>(null);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        "/api/data/wisecubation-modules?activeOnly=false"
      );
  ...
  const openDialog = (item?: WisecubationModuleItem) => {
    if (item) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(selectedItem ? "Module updated!" : "Module added!");
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `/api/data/wisecubation-modules?id=${selectedItem.id}`,
        { method: "DELETE" }
      );
      if (!response.ok)
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-1">
          Manage program modules for the Wisecubation page.
        </p>
      </div>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500 mb-2">
                Custom background image for this module card. Leave empty to use
                default.
              </p>
              <ImageUpload
                ...
                folder={R2_FOLDERS.WISECUBATION_MODULES}
              />
            </div>
            <div className="space-y-2">
